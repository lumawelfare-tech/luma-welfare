-- Scrub KRA PIN and identity PDFs when a closed member is anonymized.
-- Additive: does not drop financial rows, claims, contributions, or ledger.
...
BEGIN
  ...
  SELECT coalesce(array_agg(md.storage_path), ARRAY[]::text[])
  INTO v_identity_paths
  ...
  IF NOT v_has_financial THEN
    ...
    RETURN jsonb_build_object(
      ...
      'member_document_paths', to_jsonb(v_identity_paths),

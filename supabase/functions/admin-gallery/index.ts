...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method !== 'GET') {
      const rl = await rateLimitAsync(req, 'admin-gallery-mutation', { userId: session.id, adminClient })
    ...
    if (req.method === 'GET' && !isIdPath) {
      ...
      if (q) {
        const orFilter = buildIlikeOrFilter(['title', 'caption'], q)
    ...
    return handleAdminError(err, 'admin-gallery')
  }
})

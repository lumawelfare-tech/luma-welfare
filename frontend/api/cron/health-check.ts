...
function getEnvOrThrow(key: string): string {
  if (key === 'SUPABASE_SERVICE_KEY') {
    const value = getServiceKey()
    if (!value) throw new Error('Missing environment variable: SUPABASE_SERVICE_KEY (or SUPABASE_SERVICE_ROLE_KEY)')
    return value
  }
 ...
 * Shallow check is public; detail=true requires CRON_SECRET (fail-closed on Edge).
 */
...
export default async function handler(
  ...
  const verifiedCronSecret = cronSecret
  ...
  try {
    ...
    const [healthResult, adminResult, dbResult] = await Promise.all([
      checkHealthEndpoint(supabaseUrl, verifiedCronSecret),

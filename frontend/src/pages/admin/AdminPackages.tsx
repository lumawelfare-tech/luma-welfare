...
export function AdminPackages() {
  ...
  async function saveRules(id: string) {
    ...
    try {
      ...
      setNotice('Rules saved. The qualification engine reads these on the next evaluation.')
      await load()
    } catch (e) {
      setError(e instanceof ApiError ? e.message : 'Rules must be valid JSON.')
    }
  }
  ...
  async function addPackage(e: React.FormEvent) {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.message : 'Could not add the package.')
    }
  }
  ...
  return (
    <div className="container-luma py-10">
      ...
      {showForm && (
        <form onSubmit={addPackage} className="mt-6 max-w-lg rounded-2xl border border-stone-200 bg-white p-6">
          ...
          <div className="mt-4 space-y-3">
            <input value={newPkg.code} onChange={(e) => setNewPkg((p) => ({ ...p, code: e.target.value }))} placeholder="code (e.g. water_drilling)" className="w-full rounded-md border border-stone-300 px-3 py-2 text-sm outline-none focus:border-luma-500" />
            <input value={newPkg.name} onChange={(e) => setNewPkg((p) => ({ ...p, name: e.target.value }))} placeholder="Name" className="w-full rounded-md border border-stone-300 px-3 py-2 text-sm outline-none focus:border-luma-500" />
      ...
      {notice && <p className="mt-4 rounded-md bg-luma-50 px-3 py-2 text-sm text-luma-800">{notice}</p>}
        ...
        {packages.map((p) => (
          <div key={p.id} className={`rounded-2xl border bg-white p-5 ${p.is_active ? 'border-stone-200' : 'border-stone-300 opacity-70'}`}>
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              {p.is_active && (
                <button onClick={() => retire(p.id)} className="rounded-md border border-stone-300 px-3 py-1.5 text-xs font-medium text-stone-600 hover:bg-stone-100">
                  Retire
                </button>
              )}
            </div>

  ...
  async function approve(sub: Subscription) {
    ...
    try {
      await api(`/admin/subscriptions/${sub.id}`, {
        method: 'PATCH',
        auth: true,
        body: { status: 'active', reason: dialogReason.trim() || undefined },
      })
      setNotice(`Subscription for ${sub.members?.full_name ?? 'member'} activated.`)
      setApproveTarget(null)
      setDialogReason('')
      await load()
    } catch (e) {
      setError(e instanceof ApiError ? e.message : 'Could not activate subscription.')
    } finally {
      setBusyId(null)
    }
  }
  ...
  async function suspend(sub: Subscription) {
    ...
    try {
      await api(`/admin/subscriptions/${sub.id}`, {
        method: 'PATCH',
        auth: true,
        body: { status: 'paused', reason: dialogReason.trim() || undefined },
      })
      setNotice(`Subscription for ${sub.members?.full_name ?? 'member'} suspended.`)
      setSuspendTarget(null)
      setDialogReason('')
      await load()
    } catch (e) {
      setError(e instanceof ApiError ? e.message : 'Could not suspend subscription.')
    } finally {
      setBusyId(null)
    }
  }
  ...
  async function cancel(sub: Subscription) {
    ...
    try {
      await api(`/admin/subscriptions/${sub.id}`, {
        method: 'PATCH',
        auth: true,
        body: { status: 'cancelled', reason: dialogReason.trim() || undefined },
      })
      setNotice(`Subscription for ${sub.members?.full_name ?? 'member'} cancelled.`)
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white">
          ...
          <tbody>
            {subs.map((s) => (
              <tr key={s.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-gray-600">{s.packages?.name ?? '—'}</td>
                <td className="px-4 py-3 text-gray-500 text-xs">{s.package_tiers?.name ?? '—'}</td>
                  ...
                  <div className="flex justify-end gap-1.5">
                    {s.status === 'pending' && (
                      <button
                        ...
                        onClick={() => { setApproveTarget(s); setDialogReason('') }}
                    ...
                    {s.status === 'active' && (
                      <>
                        <button
                          disabled={busyId === s.id}
                          onClick={() => { setSuspendTarget(s); setDialogReason('') }}
                        ...
                        <button
                          disabled={busyId === s.id}
                          onClick={() => { setCancelTarget(s); setDialogReason('') }}
        ...
        {subs.length === 0 && (
          <div className="p-12 text-center text-gray-500">
            <p className="text-sm">No {filter ? filter.toLowerCase() : ''} subscriptions found.</p>
          </div>
        )}
      </div>
          ...
          <div className="w-full max-w-md rounded-xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="px-6 py-5">
              ...
              <p className="mt-1 text-sm text-gray-500">
                {approveTarget.status === 'paused' ? 'Reactivate' : 'Activate'} the subscription for{' '}
                <strong>{approveTarget.members?.full_name ?? 'member'}</strong> ({approveTarget.packages?.name ?? 'package'})?
              </p>
              <div className="mt-4">
            ...
            <div className="px-6 py-5">
              ...
              <div className="mt-4">
                ...
                <textarea
                  value={dialogReason}
                  onChange={(e) => setDialogReason(e.target.value)}
                  placeholder="e.g. Member request, policy violation…"
                  rows={3}
                  className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-luma-500 resize-none"
                />
              </div>
